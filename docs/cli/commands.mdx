...
<Note>
  ...
  agent can run commands on it and reach its local MCP servers and files. It has
  its own commands and setup — see the [Device Bridge](/cli/device-bridge)
  guide. On macOS you can also connect the machine you're on with one click from
  the GAIA desktop app, no CLI required.
</Note>
