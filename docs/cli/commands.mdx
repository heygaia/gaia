...
<Note>
  `gaia bridge` connects your machine's local MCP servers and files to GAIA over
  a secure outbound tunnel. It has its own commands and setup — see the
  [Device Bridge](/cli/device-bridge) guide.
</Note>
