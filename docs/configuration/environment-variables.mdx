...
<CardGroup cols={2}>
  <Card title="Backend Environment Variables">
    Configure API keys, database connections, and backend services for the GAIA
    backend application
  </Card>
  <Card title="Frontend Environment Variables">
    Set up API URLs, authentication secrets, and frontend configuration for the
    GAIA web interface
  </Card>
</CardGroup>
...
<Note>
  **Local Configuration**: Frontend environment variables are typically
  configured locally as they're needed during the build process.
</Note>
