...
GAIA uses many integrations, but you don’t need all keys to start. We load env via Pydantic settings and lazily initialize providers only when first used. See: [Lazy loading env & providers](/developers/extra/lazy-loading).
...
- **Search & weather**: `BING_API_KEY`, `OPENWEATHER_API_KEY`
...
## See also
- [Lazy loading env & providers](/developers/extra/lazy-loading)
