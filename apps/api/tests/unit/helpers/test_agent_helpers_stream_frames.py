...
def _reasoning_chunk(text: str, *, content: str = "") -> AIMessageChunk:
    """A chunk carrying provider-style thinking, as ChatOpenRouter streams it."""
...
async def test_comms_thinking_is_streamed_as_a_top_level_reasoning_frame() -> None:
    """Without this frame the user watches a frozen UI while comms reasons. No
    ``subagent_id``: this is the root turn, which the client renders as a
    top-level thinking block rather than nesting it under a delegate."""
    state = _StreamAccumulators()
...
async def test_a_chunk_with_no_thinking_emits_no_reasoning_frame() -> None:
    ...
    frame each would be a per-token spinner the client cannot tell from real work."""
    state = _StreamAccumulators()
