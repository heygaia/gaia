...
class TestInitializeChromaTriggersStore:
    ...
    async def test_batch_write_failure_does_not_abort_startup(self):
        ...
        The triggers store is registered strategy=ERROR + auto_initialize=True, so
        a ChromaBatchWriteError escaping initialize_chroma_triggers_store would
        abort app boot. run_index_warmup must swallow it and still return the store.
        """
        upsert = [
            (
                "new_slug",
