...
class TestWorkflowsTriggersAndSystem:
    ...
    async def test_reset_repersists_the_top_level_schedule_fields(
        ...
        The re-arm path reads the top-level fields: ``_rearm_if_scheduled``
        gates on ``workflow.repeat`` and ``handle_recurring_task`` computes
        every next occurrence from it. Rewriting only ``trigger_config``
        ...
        wf = await repo.create(
            _workflow(
                ...
                trigger_config=TriggerConfig(
                    type=TriggerType.SCHEDULE, enabled=True, cron_expression="0 3 * * *"
                ),
            )
        )
        assert wf.repeat == "0 3 * * *"
        ...
        assert raw is not None and raw["repeat"] == "0 9 * * *"
