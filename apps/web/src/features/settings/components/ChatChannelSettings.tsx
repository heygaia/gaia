 ...
 * plus the daily activation opt-out that rides on the platform at the top.
 */
export function ChatChannelSettings({
  linkedPlatforms,
}: ChatChannelSettingsProps) {
  const { linkedOrder, dailyCheckIns, saving, move, setDailyCheckIns } =
    useChatChannelSettings(linkedPlatforms);
  ...
  return (
    <>
      ...
      <SettingsSection>
        <SettingsRow
          label="Daily check-ins for your first week"
          description="One message a day, on the platform above, for your first five days. Reply stop anytime."
        >
          <Switch
            size="sm"
            isSelected={dailyCheckIns}
            onValueChange={setDailyCheckIns}
            aria-label="Daily check-ins for your first week"
          />
        </SettingsRow>
      </SettingsSection>
    </>
