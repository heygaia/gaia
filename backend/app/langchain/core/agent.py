...
async def call_workflow_agent(
    ...
    """
    Process workflow instruction with AI agent using the registered workflow processing graph.
    ...
    """
    logger.info(
        f"Starting workflow processing for user {user_id} using registered graph"
    )

    try:
        # Get the registered workflow processing graph from GraphManager
        graph = await GraphManager.get_graph("workflow_processing")

        if not graph:
            logger.error(f"No workflow processing graph found for user {user_id}")
            raise ValueError(f"Workflow processing graph not found: {user_id}")

        logger.info(
            f"Workflow processing graph retrieved successfully for user {user_id}"
        )

        # Convert old messages to context for metadata
        ...
        # Create initial state for the workflow processing graph with runtime configuration
        initial_state = {
            "input": instruction,
            ...
            "metadata": {
                ...
                "persist_workflow": True,  # Default to persisting workflow state
                "skip_planning": False,  # Default to performing planning
                "skip_replanning": False,  # Default to performing replanning
                "force_new_plan": False,  # Default to using existing plans if available
            },
        }

        # Execute the workflow using the registered plan-execute graph
        result = await graph.ainvoke(
            ...
            config={
                ...
                "recursion_limit": 20,  # Reasonable limit for workflow processing
                "metadata": {
                    "user_id": user_id,
        ...
        logger.info(f"Workflow processing completed successfully for user {user_id}")
        ...
        if not final_response:
            # Fallback to a summary of the workflow execution
            final_response = f"Workflow executed successfully. Executed {len(result.get('past_steps', []))} steps."
